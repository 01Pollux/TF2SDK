...
public:
	...
	virtual const Vector3D_F& OBBMinsPreScaled() const abstract;
	virtual const Vector3D_F& OBBMaxsPreScaled() const abstract;
	virtual const Vector3D_F& OBBMins() const abstract;
	virtual const Vector3D_F& OBBMaxs() const abstract;
	...
	virtual const Vector3D_F& GetCollisionOrigin() const abstract;
	virtual const Vector3D_F& GetCollisionAngles() const abstract;
	virtual const Matrix3x4_F& CollisionToWorldTransform() const abstract;
	...
	virtual Const::EntCollisionGroup GetCollisionGroup() const abstract;
	...
	virtual const Matrix3x4_F* GetRootParentToWorldTransform() const abstract;
};
...
public:
	...
	// Data accessors
	virtual const Vector3D_F&	GetRenderOrigin() abstract;
	virtual const Angle_F&		GetRenderAngles() abstract;
	...
	virtual int DrawModel(int studio_draw_flags) abstract;
	...
	virtual bool	SetupBones(Matrix3x4_F* pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime) abstract;
	...
	virtual bool	GetShadowCastDirection(Vector3D_F* pDirection, int shadowType) const abstract;
	...
	virtual const Matrix3x4_F& RenderableToWorldTransform() abstract;

	// Attachments
	virtual int LookupAttachment(const char* pAttachmentName) abstract;
	virtual	bool GetAttachment(int number, Vector3D_F& origin, Angle_F& angles) abstract;
	virtual bool GetAttachment(int number, Matrix3x4_F& matrix) abstract;
