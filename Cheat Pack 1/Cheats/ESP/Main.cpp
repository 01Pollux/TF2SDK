...
{
	m_DisplayFont = px::ImGuiLoader->FindFont("Arimo-Medium, s=14");
	px::ImGuiLoader->AddCallback(px::ThisPlugin, "ESP", std::bind(&GlobalESP::OnRender, this));
...
{
	using namespace tf2;
	if (px::ThisPlugin->IsPluginPaused() || ITFPlayerInternal::BadLocal())
		return;

	PX_PROFILE_SECTION("Visuals", "GlobalESP::OnDrawESP");
