...
public:
	...
	{
		if constexpr (requires(reference v) { std::end(v); })
			return std::end(m_Value);
		else
			return begin() + sizeof(value_type);
	}
...
#define SG_COMMAND(Name, ...)													\
static const char* Name##_callback(ConCommand* pCmd, const CommandArgs& args);	\
static SG::ConCommand Name##_cmd{												\
	#Name,																		\
	Name##_callback,															\
	__VA_ARGS__																	\
};																				\
const char* Name##_callback(ConCommand* pCmd, const CommandArgs& args)
